...
- 2026-09-19: Corrección native review R1-001 (endpoint claim anónimo): throttling in-memory POR INSTANCIA — token bucket por IP (10/60 s) + tope de 3 claims en vuelo por IP (ventana = TTL 15 min) + sweep con intervalo mínimo de 60 s desacoplado del request path. Semántica best-effort por instancia (no global entre instancias serverless), sin dependencias. → hash en commit de tracking
