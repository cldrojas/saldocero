...
export function setupBudget(
  ...
  return inTx(db, () => {
    ...
    // Upsert daily account FIRST: la transacción inicial referencia account_id
    // 'daily' y con foreign_keys=ON (initDb vía upgradeDb) insertarla antes
    // rompería la FK y con ella toda la transacción de setup.
    db.run(
