...
import { flushNow, schedule } from '@/lib/db/autosave'
...
export async function commitToggleAutoSave(): Promise<number> {
  const db = await getDb()
  const next = repo.toggleAutoSave(db).autoSave
  schedule()
  return next
}
...
export async function commitClearData(): Promise<void> {
  const db = await getDb()
  repo.clearData(db)
  // clearData vacía la DB: persistir YA (sin esperar el debounce) para que un
  // reload no resucite el snapshot stale que acabamos de borrar.
  await flushNow()
}
