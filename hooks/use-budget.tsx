...
import { flushNow, subscribeToVisibility } from '@/lib/db/autosave'
...
export function useBudget() {
  ...
  useEffect(() => {
    let cancelled = false
    // La UI es 100% client-side: un reload solo vuelve a ver el estado si el
    // snapshot en IndexedDB está al día. El listener de visibilidad flushea en
    // pagehide/ocultar la pestaña, así el último cambio nunca se pierde.
  ...
  useEffect(() => {
    ...
    // El rollover (potencial transferencia de leftover) ya se encola vía
    // commitTransferFunds → schedule(); forzamos el persist YA porque el
    // cambio de día es un evento discreto con riesgo de perderse.
    void flushNow()
