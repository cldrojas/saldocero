...
// ─── Autosave mock (mirrors the GREEN wiring: regular commits schedule(), ─
// clearData flushes immediately, boot subscribes the visibility listener) ─
const autosaveMock = vi.hoisted(() => ({
  schedule: vi.fn(),
  flushNow: vi.fn(),
  subscribeToVisibility: vi.fn(),
}))
...
const mockedCommits = vi.hoisted(() => {
  ...
  return {
    ...
    commitClearData: vi.fn(async () => {
      autosaveMock.flushNow()
      return { success: true }
    }),
...
vi.mock('@/lib/db/autosave', () => autosaveMock)
...
describe('useBudget hook (SQLite-backed)', () => {
  ...
  it('autosave: setup y addTransaction posponen un snapshot (schedule), clearData flushea ya (flushNow)', async () => {
    const { result } = renderHook(() => useBudget())
    await waitFor(() => expect(result.current.accounts).toHaveLength(0))
    ...
    await waitFor(() => expect(autosaveMock.schedule).toHaveBeenCalled())

    const daily = result.current.accounts.find((a) => a.type === 'daily')!
    act(() => {
      result.current.addTransaction({ type: 'expense', amount: 200, description: 'Coffee', account: daily.id })
    })
    await waitFor(() => expect(autosaveMock.schedule.mock.calls.length).toBeGreaterThanOrEqual(2))

    const scheduleCallsBeforeClear = autosaveMock.schedule.mock.calls.length
    act(() => {
      result.current.clearData()
    })
    await waitFor(() => expect(autosaveMock.flushNow).toHaveBeenCalled())
    // clearData no pospone un nuevo snapshot
    expect(autosaveMock.schedule.mock.calls.length).toBe(scheduleCallsBeforeClear)
  })

  it('autosave: el boot suscribe el listener de visibilidad para flushear en pagehide', async () => {
    renderHook(() => useBudget())
    await waitFor(() => expect(autosaveMock.subscribeToVisibility).toHaveBeenCalledTimes(1))
  })
})